...
def main():
    ...
    print("="*70)
    print(f"  🚀 Whisper Base Model 평가 (011)")
    print(f"  모델: {MODEL_SIZE} | 디바이스: {DEVICE} | Compute: {COMPUTE_TYPE}")
    ...
    df_off.to_csv("results_base_011_vad_off.csv", index=False, encoding="utf-8-sig")
    print(f"\n  💾 results_base_011_vad_off.csv")
    print_summary(df_off, "BASE 011 - VAD OFF")
    gc.collect()

    # VAD ON
    results_on = evaluate_mode(model, wav_files, SAMPLES_DIR, True, "VAD ON", jiwer)
    df_on = pd.DataFrame(results_on)
    df_on.to_csv("results_base_011_vad_on.csv", index=False, encoding="utf-8-sig")
    print(f"\n  💾 results_base_011_vad_on.csv")
    print_summary(df_on, "BASE 011 - VAD ON")
