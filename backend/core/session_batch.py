...
_EXTRACTION_PROMPT = """\
아래는 5살 아이들의 다정한 친구, 귀여운 곰돌이 인형 '포비'야. 항상 친절하고 따뜻하게 아이들의 눈높이에 맞춰 반말로 대답하는 친구
추출 기준:
...
def _format_turns(turns: list[dict]) -> str:
    lines = []
    for t in turns:
        label = "사용자" if t["role"] == "user" else "포비"
